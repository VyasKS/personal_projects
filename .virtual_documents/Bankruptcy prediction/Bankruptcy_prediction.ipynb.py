...
get_ipython().getoutput("ln -sf /opt/bin/nvidia-smi /usr/bin/nvidia-smi")
get_ipython().getoutput("pip install gputil")
get_ipython().getoutput("pip install psutil")
get_ipython().getoutput("pip install humanize")
import psutil
import humanize
import os
...
from sklearn.impute import SimpleImputer                                                # Using this in place of MICE (Multiple Imputation by Chained Equations)
...
def perform_data_modeling(_models_, _imputers_, verbose=False, k_folds=5):
    ...
    for model_name, clf in _models_.items():
        ...
        for imputer_name, dataframes_list in _imputers_.items():
            ...
            feature_dfs, label_dfs = OVERSAMPLING().split_dataframes_features_labels(dfs=dataframes_list)            
...
for i in range(len(results_by_estimators)):
...
estimators = [i+1 for i in range(len(results_by_estimators))] 
